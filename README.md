...
- **AI 看到的就是 informer 看到的**：诊断期间 agent 只有三个工具——读订阅配置与失败原因、**按 informer 自己的方式**抓取页面原文（带上该订阅的 curl 请求头和全局代理）、用候选配置真实试跑一次解析。它被明确禁止用别的方式访问该页面：换一种抓法拿到的字节不一样，「页面格式变了」这类判断就会失真。
...
- **实现方式**：informer 把这三个工具做成一个 MCP server，由 agent 命令行以 `--strict-mcp-config` 加载——也就是说这次会话能碰到的工具**只有**这三个，既够不到网络搜索，也够不到数据库（工具进程读的是一份临时快照文件，运行结束即删除）。
