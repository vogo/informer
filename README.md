...
| Tab | 能做什么 |
| --- | --- |
| **订阅** | 左侧分类树：新增 / 编辑 / 删除分类，用整数「排序值」决定顺序（越小越靠前，相同排序值按分类 ID 排）。右侧按分类筛选的订阅卡片：展示解析类型、抓取状态、所属分类、错误信息，卡片右上角的开关直接启停订阅，底部可「测试抓取」（真实抓取，但不写库、不改订阅状态）、编辑、删除；抓取失败的订阅上还会多出一个「AI 诊断修复」按钮，见下文。测试抓取的抽屉底部有「执行日志」折叠面板，抓取过程中实时输出：请求地址与 HTTP 状态码、正则/JSON 路径的匹配情况、逐条解析出的标题与链接；agent 类型还会显示完整提示词、agent 每一次搜索与网页抓取、以及返回的原文。抓取中面板自动展开（日志就是进度），成功后自动折叠，失败则保持展开。右上角可「导出 / 导入」全部订阅配置，见下文。新建订阅弹窗右上角还有「AI 定义配置」，用对话的方式让 AI 试出一份能用的配置，见下文。 |
...
网页改版之后，订阅配的正则或 JSON 路径就不再匹配，卡片变成「抓取失败」。抓取失败的订阅卡片上有「AI 诊断修复」按钮，点开后由配置好的 agent（同「设置」里那套接口地址 / 密钥 / 模型）读页面、找原因、试配置：

- **AI 看到的就是 informer 看到的**：诊断期间 agent 有三个 informer 自己的工具——读订阅配置与失败原因、**按 informer 自己的方式**抓取页面原文（带上该订阅的 curl 请求头和全局代理）、用候选配置真实试跑一次解析。
- **探索与判定是分开的**：agent 也可以用 WebSearch / WebFetch 去**探索**——找这个站点的 feed 地址、确认它是不是改版了、查接口文档，一个只能盯着一份文档看的诊断没法形成假设。但**判定必须落回 informer 自己的字节**：任何结论都要以上面那个抓取工具取到的内容为准，任何候选配置都要真的试跑过。informer 抓取时带着订阅自己的请求头和代理，换一种抓法拿到的字节不一样，据此做的「页面格式变了」的判断就是错的。
- **可以反复试**：试跑不写任何数据，agent 可以「改正则 → 试 → 再改」直到解析出合理的文章列表，这正是正则调试真正花时间的地方。
- **AI 没有保存配置的能力**。它只给出结论和建议的改动；informer 拿到建议后**自己再复核一次**（用建议的配置真跑一遍解析），复核解析出文章才会亮出「应用修复」按钮。agent 自称修好、而复核没通过时，面板会明说这件事并且不提供一键应用。
- **改动是逐字段展示的**：抽屉里列出每个字段的「原值 → 新值」，以及复核解析出的前几条标题（请确认那确实是文章，而不是导航栏或广告）。确认之后点「应用修复」才写库，并立刻重新抓取一次——修好了状态就恢复正常，仍然失败则保留失败状态和新的错误信息。
- **能改的字段是有限的**：只有订阅地址、自定义请求、解析类型及其参数（正则 / 表达式 / JSON 路径 / Agent 提示词）和链接重定向；标题、分类、权重、启用状态不在它的职责范围内。
- **修不好也会说清楚**：站点关闭、需要登录、内容改由 JS 渲染等等，面板会给出结论和建议（换订阅地址、改用 agent 类型抓取、或者干脆删掉这个订阅），而不是硬凑一个「能匹配到东西」的正则。
- 诊断通常要一到几分钟，抽屉底部的「诊断日志」实时输出每一步：重试当前配置、agent 每一次工具调用、复核结果。同一时间只允许跑一个诊断。
- **实现方式**：informer 把这三个工具做成一个 MCP server，由 agent 命令行以 `--strict-mcp-config` 加载——也就是说除了这三个工具和只读的 WebSearch / WebFetch 之外，这次会话再碰不到别的东西，尤其**够不到数据库**（工具进程读的是一份临时快照文件，运行结束即删除）。

### AI 定义配置

新建一个订阅，难的从来不是填表，而是要自己判断这个站点该用哪种解析方式、然后把正则或 JSON 路径写对。新建订阅弹窗右上角的「AI 定义配置」把这一步交给同一个 agent：点开之后弹窗变成一个聊天界面，你用大白话说清楚想订阅哪里的什么内容，剩下的它来试。

- **它按固定顺序选解析方式**，不是凭感觉：先试标准 feed（把你给的地址原样试一遍，再去页面 `<head>` 里找 `<link rel="alternate">`，再依次探 `/feed`、`/rss`、`/atom.xml` 等常见路径）——feed 是站点改版后唯一还能继续工作的形态，所以必须先排除；抓到的响应体是 JSON 就用 JSON 路径而不是正则；HTML 列表页才写正则；只有压根没有可抓地址、或内容由 JS 渲染时，才退到 agent 类型（那意味着**每次定时抓取都要真跑一次 AI**，慢且贵）。
- **工具和诊断是同一套**：按 informer 自己的方式抓页面原文、用完整候选配置真实试跑一次解析，外加只读的 WebSearch / WebFetch 用来探索（例如去找某个博客的 feed 地址）。同样是探索归探索、判定归 informer 的字节。
- **「解析出东西」不算成功**：标题必须是文章标题而不是导航栏、分页或广告；链接必须是每篇文章各自的地址——解析出好几条却都指向同一个地址，会被当场判为失败。
- **AI 依然没有保存的能力**：它把配置通过一个专门的工具交给 informer（在回复里用文字描述配置不算数），informer 拿到之后**自己再跑一遍解析**核对，核对通过才会在聊天里出现「保存并新建订阅」按钮。点了才写库。
- **提案是逐字段展示的**：解析类型、地址、正则/路径逐条列出，下面是复核解析出的前几条标题——请确认那确实是文章。旁边可以选分类（也可以直接输入新分类名），标题用 AI 起的那个，不满意可以改。
- **agent 类型的提案不做自动复核**，因为那会在当前 agent 里再启动一个 agent。这种提案会明确标注「未复核」，仍然允许保存，但保存后请用订阅卡片上的「测试抓取」验证一次。
- 每一轮通常要一到几分钟，聊天下方的「执行日志」实时输出 agent 的每一次抓取和试跑。同一时间只允许跑一个 AI 任务（诊断和这里共用一个闸），一次对话最多 12 轮——每轮都会把整段对话重发一次，太长的对话不如重新开始。
