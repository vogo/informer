...
// Package parsecfg is the part of a subscription that decides how its page is
// turned into articles, and the tools for trying one out.
//
// Two features need exactly this and nothing else: repairing a source whose
// parse broke, and composing a source that does not exist yet. Both hand an
// agent the same loop - read the bytes informer received, propose a candidate
// configuration, parse with it, look at what came out - and both must speak the
// same field names, offer the same trial, and refuse in the same way. Declaring
// that vocabulary once is what keeps them from drifting into two dialects of the
// same thing.
//
// Nothing here can write to the database. A candidate is applied to a copy, the
// parse runs in memory, and the result is text an agent reads. Turning a
// candidate into a stored subscription is a decision that belongs to a person,
// taken elsewhere.
package parsecfg
...
// Changes is a set of parse affecting fields proposed for a source.
//
...
// enabled flag are absent because none of them can decide whether a parse works,
...
// FieldChange is one field a proposal would edit, in the shape a person reads
// it: what it is now and what it would become.
...
// FieldValue is one configuration field of a source, rendered for reading: a
// string is quoted so an empty one is visible, a bool is written out.
type FieldValue struct {
	// Field is the json name of the column.
	Field string

	// Text is the stored value, already rendered.
	Text string
}

// IsEmpty reports whether the proposal carries no edit at all.
...
// fieldTable is the single place a parse affecting column is declared. Apply,
// the diff, the tool schemas and the prompts' field lists all read from it, so a
// column added here needs no matching edit anywhere else.
//
...
// Fields lists the columns a proposal may set, in declaration order. The prompts
// state this list, so what the agent is allowed to touch and what it is told
...
// Describe renders every column of a source in declaration order, so a tool that
// shows a configuration shows all of it - including the fields that are empty,
// which is often the answer.
func Describe(source *feed.Source) []FieldValue {
	values := make([]FieldValue, 0, len(fieldTable))

	for _, entry := range fieldTable {
		var text string

		if entry.stringP != nil {
			text = strconv.Quote(entry.getStr(source))
		} else {
			text = strconv.FormatBool(entry.getBool(source))
		}

		values = append(values, FieldValue{Field: entry.name, Text: text})
	}
...
// with, and only an explicit save by a person turns it into a stored one.
...
// proposedText renders what the proposal would put there instead.
...
// place a newly added column could be forgotten.
